...
CREATE POLICY IF NOT EXISTS withdrawal_destinations_customer_select ON withdrawal_destinations
...
CREATE POLICY IF NOT EXISTS withdrawal_destinations_customer_insert ON withdrawal_destinations
  FOR INSERT TO authenticated
  WITH CHECK (user_id = auth.uid());

-- Customers can update their own destinations (before verification)
CREATE POLICY IF NOT EXISTS withdrawal_destinations_customer_update ON withdrawal_destinations
  FOR UPDATE TO authenticated
  USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());

-- Customers can soft-delete their own destinations
CREATE POLICY IF NOT EXISTS withdrawal_destinations_customer_delete ON withdrawal_destinations
...
CREATE POLICY IF NOT EXISTS withdrawal_destination_versions_customer_select ON withdrawal_destination_versions
  FOR SELECT TO authenticated
  USING (
    user_id = auth.uid()
    OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin'
...
CREATE POLICY IF NOT EXISTS withdrawal_destination_versions_admin_insert ON withdrawal_destination_versions
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Versions are immutable for everyone
CREATE POLICY IF NOT EXISTS withdrawal_destination_versions_no_update ON withdrawal_destination_versions
  FOR UPDATE TO authenticated
  USING (false);

CREATE POLICY IF NOT EXISTS withdrawal_destination_versions_no_delete ON withdrawal_destination_versions
  FOR DELETE TO authenticated
  USING (false);
...
CREATE TABLE IF NOT EXISTS gbp_withdrawals (
  ...
  destination_version_id uuid not null references withdrawal_destination_versions(id) on delete restrict,
...
CREATE POLICY IF NOT EXISTS gbp_withdrawals_customer_select ON gbp_withdrawals
  FOR SELECT TO authenticated
  USING (
    user_id = auth.uid()
    OR (SELECT role FROM profiles WHERE id = auth.uid()) = 'admin'
...
CREATE POLICY IF NOT EXISTS gbp_withdrawals_admin_insert ON gbp_withdrawals
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Only admins update (via approve/reject/fail RPCs)
CREATE POLICY IF NOT EXISTS gbp_withdrawals_admin_update ON gbp_withdrawals
  FOR UPDATE TO authenticated
...
CREATE POLICY IF NOT EXISTS admin_privilege_audit_admin_select ON admin_privilege_audit
  FOR SELECT TO authenticated
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Only system RPCs insert
CREATE POLICY IF NOT EXISTS admin_privilege_audit_admin_insert ON admin_privilege_audit
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Never allow customer update/delete
CREATE POLICY IF NOT EXISTS admin_privilege_audit_no_update ON admin_privilege_audit
  FOR UPDATE TO authenticated
  USING (false);

CREATE POLICY IF NOT EXISTS admin_privilege_audit_no_delete ON admin_privilege_audit
  FOR DELETE TO authenticated
  USING (false);

-- ── Updated_at triggers for mutable tables ──────────────────────
CREATE TRIGGER IF NOT EXISTS trg_withdrawal_destinations_updated
  BEFORE UPDATE ON withdrawal_destinations
  FOR EACH ROW
  EXECUTE FUNCTION update_updated_at();

CREATE TRIGGER IF NOT EXISTS trg_gbp_withdrawals_updated
