...
DROP POLICY IF EXISTS notification_events_customer_select ON notification_events;
CREATE POLICY notification_events_customer_select ON notification_events
...
DROP POLICY IF EXISTS notification_events_system_insert ON notification_events;
CREATE POLICY notification_events_system_insert ON notification_events
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Only notification worker updates delivery status
DROP POLICY IF EXISTS notification_events_admin_update ON notification_events;
CREATE POLICY notification_events_admin_update ON notification_events
  FOR UPDATE TO authenticated
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin')
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Prevent event deletion (immutable audit trail)
DROP POLICY IF EXISTS notification_events_no_delete ON notification_events;
CREATE POLICY notification_events_no_delete ON notification_events
...
DROP POLICY IF EXISTS notification_delivery_log_admin_select ON notification_delivery_log;
CREATE POLICY notification_delivery_log_admin_select ON notification_delivery_log
  FOR SELECT TO authenticated
  USING ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Only system processes insert logs
DROP POLICY IF EXISTS notification_delivery_log_system_insert ON notification_delivery_log;
CREATE POLICY notification_delivery_log_system_insert ON notification_delivery_log
  FOR INSERT TO authenticated
  WITH CHECK ((SELECT role FROM profiles WHERE id = auth.uid()) = 'admin');

-- Delivery logs are immutable (audit trail)
DROP POLICY IF EXISTS notification_delivery_log_no_update ON notification_delivery_log;
CREATE POLICY notification_delivery_log_no_update ON notification_delivery_log
  FOR UPDATE TO authenticated
  USING (false);

DROP POLICY IF EXISTS notification_delivery_log_no_delete ON notification_delivery_log;
