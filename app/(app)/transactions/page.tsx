...
const TYPE_META: Record<string, { icon: string; bg: string; fg: string; label: string }> = {
  ...
  reward:        { icon: 'bolt',         bg: 'rgba(124,92,255,0.18)', fg: '#C9BBFF', label: 'Reward' },
  ...
  conversion:    { icon: 'swap_horiz',   bg: 'rgba(110,139,255,0.16)', fg: '#6E8BFF', label: 'Conversion' },
};
...
const FILTERS = ['All', 'Rewards', 'Deposits', 'Withdrawals', 'Adjustments'];
...
function fmtAmount(t: Txn, ethPrice: number): { primary: string; secondary: string; positive: boolean } {
  const gbp = t.amount_gbp ?? 0;
  const eth = t.amount_eth ?? 0;

  if (gbp !== 0) {
    const positive = gbp > 0;
    return {
      primary: `${positive ? '+' : '-'}£${Math.abs(gbp).toFixed(2)}`,
      secondary: '',
      positive,
    };
  }
  ...
  return {
    primary: `${positive ? '+' : ''}${eth.toFixed(6)} ETH`,
    secondary: `£${Math.abs(eth * ethPrice).toFixed(2)}`,
    positive,
  };
}
...
export default function TransactionsPage() {
  ...
  const filtered = FILTER_TYPE[filter]
    ? txns.filter(t => {
        if (filter === 'Rewards') return t.type === 'reward' || t.type === 'mining_reward';
        return t.type === FILTER_TYPE[filter];
      })
    : txns;
  ...
  return (
    <div style={{ maxWidth: '1100px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '14px', marginBottom: '18px', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: '0', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '999px', padding: '4px', flexWrap: 'wrap' }}>
          {FILTERS.map(f => (
        ...
        <RippleButton variant="ghost" onClick={() => {
          ...
          const rows = [['Date', 'Type', 'Amount GBP', 'Amount ETH', 'Status', 'Description']];
          txns.forEach(t => rows.push([
            new Date(t.created_at).toISOString().slice(0, 10),
            t.type,
            t.amount_gbp != null ? t.amount_gbp.toFixed(2) : '',
            (t.amount_eth ?? 0).toString(),
            t.status,
            t.description || '',
          ]));
      ...
      <div style={{ borderRadius: '24px', overflow: 'hidden', background: 'linear-gradient(180deg,rgba(255,255,255,0.045),rgba(255,255,255,0.015))', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="tx-table-header">
          <span>Transaction</span><span>Amount</span><span>Value</span><span>Tx ref</span><span style={{ textAlign: 'right' }}>Status</span>
        </div>
        ...
        {loading ? (
          ...
          <div style={{ padding: '64px 24px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '13px', color: '#4A4763', marginTop: '5px' }}>Your deposits, rewards, and withdrawals will appear here.</div>
          </div>
        ) : (
          filtered.map(t => {
            const m = TYPE_META[t.type] ?? { icon: 'swap_horiz', bg: 'rgba(255,255,255,0.08)', fg: '#C5C1D6', label: t.type.replace(/_/g, ' ') };
            const s = STATUS_META[t.status] ?? { color: '#8A8699', bg: 'rgba(255,255,255,0.08)' };
            const { primary, secondary, positive } = fmtAmount(t, ethPrice);
            ...
            return (
              <div key={t.id}>
                ...
                <div className="tx-table-row">
                  ...
                  <span className="sg" style={{ fontWeight: 700, fontSize: '14px', color: positive ? '#16D98A' : '#FF6B8A' }}>
                    {primary}
                  </span>
                  <span style={{ fontSize: '13px', color: '#A39FB5' }}>{secondary}</span>
                ...
                <div className="tx-card" style={{ padding: '15px 18px', borderBottom: '1px solid rgba(255,255,255,0.04)', alignItems: 'center', gap: '13px' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: '11.5px', color: '#7E7A8F', marginTop: '2px' }}>{dateStr}</div>
                    {t.description && <div style={{ fontSize: '11px', color: '#4A4763', marginTop: '2px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.description}</div>}
                  </div>
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    <div className="sg" style={{ fontWeight: 700, fontSize: '14px', color: positive ? '#16D98A' : '#FF6B8A' }}>{primary}</div>
